...
{
    public class ImageCutHelp
    {
        //裁切图片

        public static string SaveCutPic(string filename, string pPath, string pSavedPath, int pPartStartPointX, int pPartStartPointY, int pPartWidth, int pPartHeight, int pOrigStartPointX, int pOrigStartPointY, int imageWidth, int imageHeight)
        {
            using (Image originalImg = Image.FromFile(pPath))
            {
                ...
                string filePath = pSavedPath + "\\M_" + filename;
                string filePath_small = pSavedPath + "\\S_" + filename;
                ...
                Cmj_Common.ImageHelper.MakeThumbnail(filePath, filePath_small, 50, 50, "CUTHW");
