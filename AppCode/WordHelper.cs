...
namespace EducationV2.App_Code
{
    public class WordHelper
    {
        ...
        public bool FindCell(String bookmarkCell)
        {
            try
            {
                object bkObj = bookmarkCell;
                if (wApp.ActiveDocument.Bookmarks.Exists(bookmarkCell))
                {
                    wApp.ActiveDocument.Bookmarks.get_Item(ref bkObj).Select();
                    return true;
                }
        ...
        {
            Object count = 1;
            wApp.Selection.InsertRowsBelow(ref count);
        }
